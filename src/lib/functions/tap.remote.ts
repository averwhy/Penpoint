...
export const tap = form(Tap.omit({ id: true, created_at: true }), async ({ student_id, event_id }) => {
  return await submitTap(student_id, event_id);
});

export const manualTap = command(Tap.omit({ id: true, created_at: true }), async ({ student_id, event_id }) => {
  return await submitTap(student_id, event_id);
});
